...
class TokenRepositoryImpl @Inject constructor(
    ...
    override suspend fun saveToken(token: String) { sharedPreferences.edit().putString(KEY_TOKEN, token).apply() }
    override suspend fun getToken(): String? = sharedPreferences.getString(KEY_TOKEN, null)
    override suspend fun clearToken() { sharedPreferences.edit().remove(KEY_TOKEN).apply() }
}